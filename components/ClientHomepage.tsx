...
interface ClientHomepageProps {
  initialRates: MortgageRate[];
}

export default function ClientHomepage({ initialRates }: ClientHomepageProps) {
  const [isContactFormOpen, setIsContactFormOpen] = useState(false);
  const [isPreApprovalFormOpen, setIsPreApprovalFormOpen] = useState(false);
  
  const filteredRates = initialRates;
  ...
  return (
    <div className="min-h-screen relative gradient-bg">
      ...
      {/* Hero Section */}
      <section className="py-4 lg:py-6 relative hero-section">
            ...
            <div className="rounded-3xl shadow-2xl backdrop-blur-lg bg-white/20 border-2 border-white/30 p-8">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                <div>
                  ...
                  <p className="text-sm" style={{color: '#264653'}}>Updated twice weekly</p>
                </div>
              </div>
              
              {/* Rate Table */}
              <div className="w-full rate-table-container">
                      ...
                      <table className="w-full table-fixed">
                        ...
                        <tbody>
                          {filteredRates.filter(rate => rate.term !== "10 Year").map((rate, index) => (
                            <tr key={index} className="border-b hover:bg-gray-50 transition-all duration-300 hover:scale-[1.01] rate-table-row" style={{
                              borderColor: '#2A9D8F',
